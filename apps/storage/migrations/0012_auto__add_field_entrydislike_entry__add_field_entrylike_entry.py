...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'EntryDislike.entry'
        db.add_column(u'storage_entrydislike', 'entry',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, to=orm['storage.Entry'], unique=True),
                      keep_default=False)

        # Removing M2M table for field entry on 'EntryDislike'
        db.delete_table('storage_entrydislike_entry')
        ...
        # Removing M2M table for field entry on 'EntryLike'
        db.delete_table('storage_entrylike_entry')


    def backwards(self, orm):
        # Deleting field 'EntryDislike.entry'
        db.delete_column(u'storage_entrydislike', 'entry_id')

        # Adding M2M table for field entry on 'EntryDislike'
        db.create_table(u'storage_entrydislike_entry', (
        ...
        db.create_unique(u'storage_entrydislike_entry', ['entrydislike_id', 'entry_id'])

        # Deleting field 'EntryLike.entry'
        db.delete_column(u'storage_entrylike', 'entry_id')

        # Adding M2M table for field entry on 'EntryLike'
        db.create_table(u'storage_entrylike_entry', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('entrylike', models.ForeignKey(orm[u'storage.entrylike'], null=False)),
            ('entry', models.ForeignKey(orm[u'storage.entry'], null=False))
        ))
        db.create_unique(u'storage_entrylike_entry', ['entrylike_id', 'entry_id'])
