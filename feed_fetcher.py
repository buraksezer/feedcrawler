...
class ProcessEntry:
    def __init__(self, feed, options, entry, fpf):
        self.feed = feed
            ...
            for tcat in self.entry.tags:
                ...
                for zcat in qcat:
                    ...
                    if not tagname or tagname == ' ':
                        continue
                    if not models.EntryTag.objects.filter(name=tagname):
                        cobj = models.EntryTag(name=tagname)
                        try:
                            cobj.save()
                        except IntegrityError:
                            connection._rollback()
                            cobj.save()
                    fcat.append(models.EntryTag.objects.get(name=tagname))
        return fcat
    ...
    def get_entry_data(self):
        ...
        try:
            ...
            title = link
        entry_id = self.entry.get('id', title)
        ...
        if self.entry.has_key("published_parsed"):
            published_at = datetime.datetime.fromtimestamp(time.mktime(self.entry.published_parsed))
        else:
            published_at = None

        fcat = self.get_tags()

        return (link, title, entry_id, author, author_email, content,
                date_modified, fcat, published_at)

    def process(self):
        """ Process a post in a feed and saves it in the DB if necessary.
        """

        (link, title, entry_id, author, author_email, content, \
            date_modified, fcat, published_at) = self.get_entry_data()
        ...
        tobj = models.Entry(feed=self.feed, title=title, link=link,
            content=content, entry_id=entry_id, date_modified=date_modified,
            author=author, author_email=author_email, published_at=published_at)
        try:
            tobj.save()
        except IntegrityError:
            connection._rollback()
        ...
        announce_client.broadcast_group(self.feed.id, 'new_entry', data={'id': tobj.id,
            'feed_id': self.feed.id,
            'link': tobj.link,
            'published_at': tobj.published_at.strftime("%B %d, %y") if isinstance(tobj.published_at, datetime.datetime) else tobj.published_at,
...
class ProcessFeed:
    ...
    def process_entry(self, entry):
        ...
        """
        entry = ProcessEntry(self.feed, self.options, entry,
                             self.fpf)
    ...
    def process(self):
        ...
        self.feed.title = self.fpf.feed.get('title', '')[0:512]
        self.feed.tagline = self.fpf.feed.get('tagline', '')
        self.feed.link = self.fpf.feed.get('link', '')
        self.feed.last_sync = datetime.datetime.now()

        if hasattr(self.fpf.feed, "language"):
            self.feed.language = self.fpf.feed.language
        if hasattr(self.fpf, "encoding"):
            self.feed.encoding = self.fpf.encoding
        if hasattr(self.fpf.feed, "subtitle"):
            self.feed.subtitle = self.fpf.feed.subtitle
        if hasattr(self.fpf.feed, "image") and hasattr(self.fpf.feed.image, "href"):
            self.feed.image = self.fpf.feed.image.href
        ...
        if False and self.options.verbose:
            prints(u'[%d] Feed info for: %s\n' \
                ...
                self.feed.tagline, self.feed.link, self.feed.last_sync))
        ...
        for entry in self.fpf.entries:
            ...
            if models.Entry.objects.filter(feed=self.feed.id).filter(entry_id=entry_id):
                print entry_id
                break
            try:
