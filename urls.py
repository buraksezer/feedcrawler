...
urlpatterns = patterns('',
    ...
    url(r'^user/(?P<username>(?!signout|signup|signin)[\.\w-]+)/$', 'apps.frontend.views.profile_detail', name='userena_profile_detail'),
    ...
    url(r'^repost/(?P<repost_id>[\w-]+)/$', 'apps.frontend.views.repost', name='repost'),
    url(r'^list/(?P<list_slug>[-A-Za-z0-9_]+)/$', 'apps.frontend.views.list', name='list'),
    url(r'^user/(?P<username>[-A-Za-z0-9_]+)/followers$', 'apps.frontend.views.follower_list', name="follower_list"),
    url(r'^user/(?P<username>[-A-Za-z0-9_]+)/following$', 'apps.frontend.views.following_list', name="follower_list"),
...
apipatterns = patterns('',
    ...
    url(r'^api/single_repost/(?P<repost_id>[\w-]+)/$', views.SingleRepost.as_view()),
    ...
    url(r'^api/unsubscribe/(?P<feed_id>[\w-]+)/$', views.FeedUserRelationByFeedId.as_view()),
    url(r'^api/subscribe_by_id/(?P<feed_id>[\w-]+)/$', views.FeedUserRelationByFeedId.as_view()),
    ...
    url(r'^api/user_profile/(?P<username>[-A-Za-z0-9_]+)/$', views.UserProfile.as_view()),
    url(r'^api/repost_list/(?P<username>[-A-Za-z0-9_]+)/$', views.RepostList.as_view()),
    url(r'^api/repost_entry/(?P<entry_id>[\w-]+)/$', views.RepostEntryItem.as_view()),
    url(r'^api/user_fellowship/(?P<username>[-A-Za-z0-9_]+)/$', views.UserFellowship.as_view()),
